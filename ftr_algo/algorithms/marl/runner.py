...
from ftr_algo.utils.util import update_linear_schedule
...
class Runner:
    def __init__(self, vec_env, config, model_dir=""):
        ...
        if self.algorithm_name == "happo":
            from ftr_algo.algorithms.marl.happo_policy import HAPPO_Policy as Policy
            from ftr_algo.algorithms.marl.happo_trainer import HAPPO as TrainAlgo
        if self.algorithm_name == "hatrpo":
            from ftr_algo.algorithms.marl.hatrpo_policy import HATRPO_Policy as Policy
            from ftr_algo.algorithms.marl.hatrpo_trainer import HATRPO as TrainAlgo
        if self.algorithm_name == "mappo":
            from ftr_algo.algorithms.marl.mappo_policy import MAPPO_Policy as Policy
            from ftr_algo.algorithms.marl.mappo_trainer import MAPPO as TrainAlgo
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if len(done_episodes_rewards) != 0:
                ...
                self.writter.add_scalar("train/aver_rewards", aver_episode_rewards, total_num_steps)
    ...
    def log_train(self, train_infos, total_num_steps):
        ...
        for agent_id in range(self.num_agents):
            ...
            for k, v in train_infos[agent_id].items():
                ...
                self.writter.add_scalar(f"train/{agent_k}", v, total_num_steps)
    ...
    def train(self):
        ...
        for agent_id in torch.randperm(self.num_agents):
            ...
            action_prod = torch.exp(
                (new_actions_logprob.detach() - old_actions_logprob.detach())
                .reshape(self.episode_length, self.n_rollout_threads, action_dim)
    ...
    def log_env(self, env_infos, total_num_steps):
        for k, v in env_infos.items():
            self.writter.add_scalar(f"train/log_env_{k}", torch.mean(v), total_num_steps)
