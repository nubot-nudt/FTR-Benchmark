...
from ftr_algo.algorithms.marl.actor_critic import Actor, Critic
from ftr_algo.algorithms.marl.utils.popart import PopArt
from ftr_algo.algorithms.utils.util import check
from ftr_algo.utils.util import get_gard_norm, huber_loss, mse_loss
...
class HATRPO:
    ...
    def trpo_update(self, sample, update_actor=True):
        ...
        if self._use_policy_active_masks:
            loss = (
                torch.sum(ratio * factor_batch * adv_targ, dim=-1, keepdim=True) * active_masks_batch
        ...
        old_actor = Actor(self.policy.args, self.policy.obs_space, self.policy.act_space, self.device)
