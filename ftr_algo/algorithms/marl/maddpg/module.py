...
from ftr_algo.utils.util import get_gard_norm
...
class MADDPG_policy:
    def __init__(
        ...
        # print(self.actor)
        # print(self.critic)
        self.actor_optimizer = torch.optim.Adam(self.actor.pi.parameters(), lr=self.lr)
        self.critic_optimizer = torch.optim.Adam(self.critic.q.parameters(), lr=self.lr)
    ...
    def act(self, obs, deterministic=False):
        if deterministic == True:
            ...
            actions = self.actor.act(obs)
            actions = torch.clamp(
                actions + self.act_noise * torch.randn(actions.shape).to(self.device),
                -self.act_limit,
                self.act_limit,
            )
