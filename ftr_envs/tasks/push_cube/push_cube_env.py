...
@configclass
class PushCubeEnvCfg(DirectMARLEnvCfg):
    # env
    decimation = 5
    episode_length_s = 25.0
    possible_agents = ["left_robot", "center_robot", "right_robot"]
    num_actions = {"left_robot": 2, "center_robot": 2, "right_robot": 2}
    num_observations = {"left_robot": 14, "center_robot": 14, "right_robot": 14}
    num_states = -1
    ...
    robot_config = {
        ...
        "chassis_wheel_joint_stiffness": 0,
        "chassis_wheel_joint_damping": 100,

        "flipper_wheel_render_mass": 1,
        "flipper_wheel_joint_stiffness": 0,
        "flipper_wheel_joint_damping": 150,

        "flipper_pos_max": 60,
        "flipper_joint_max_vel": 90,
        "flipper_joint_stiffness": 100,
        "flipper_joint_damping": 0,
    }
    robot_render_config = {
        "flipper": {
...
class PushCubeEnv(DirectMARLEnv):
    ...
    def __init__(self, cfg: PushCubeEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _apply_action(self) -> None:
        ...
        flipper_pos = torch.ones(self.num_envs, 4, device=self.device) * 90
        self.left_robot.set_all_flipper_positions(flipper_pos, degree=True)
        self.center_robot.set_all_flipper_positions(flipper_pos, degree=True)
        self.right_robot.set_all_flipper_positions(flipper_pos, degree=True)
    ...
    def _get_rewards(self) -> dict[str, torch.Tensor]:
        ...
        # orient_diff = (torch.tensor([1, 0, 0, 0], device=self.device) - self.cube_obj.data.root_quat_w).norm(dim=-1)
        self.last_cube_pos = cube_pos.clone()
        # reward = forward_dis - 1e-3 * orient_diff
        reward = forward_dis
        return {
            "left_robot": reward - 1e-4 * (cube_pos[:, :2] - self.left_robot.positions[:, :2]).norm(dim=-1),
            "center_robot": reward - 1e-4 * (cube_pos[:, :2] - self.center_robot.positions[:, :2]).norm(dim=-1),
            "right_robot": reward - 1e-4 * (cube_pos[:, :2] - self.right_robot.positions[:, :2]).norm(dim=-1),
        }

    def _get_dones(self) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]:
        terminated = torch.zeros(self.num_envs, device=self.device, dtype=torch.int32)

        terminated[self.cube_obj.data.root_pos_w[:, 2] < -1] += 1
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self.default_cube_pos = torch.tensor([8.5, 0, 2.0], device=self.device)
        ...
        self.left_robot.write_root_pose_to_sim(torch.tensor([10, -2.0, 0.7, 0, 0, 0, 1], device=self.device),
                                               env_ids=env_ids)
        self.center_robot.write_root_pose_to_sim(torch.tensor([10, 0, 0.7, 0, 0, 0, 1], device=self.device),
                                                 env_ids=env_ids)
        self.right_robot.write_root_pose_to_sim(torch.tensor([10, 2.0, 0.7, 0, 0, 0, 1], device=self.device),
                                                env_ids=env_ids)
