...
def process_td3_bc(args, env, cfg_train, logdir):
    from ptask_ftr.algorithms.offrl.td3_bc import TD3_BC
...
def process_bcq(args, env, cfg_train, logdir):
    from ptask_ftr.algorithms.offrl.bcq import BCQ
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_iql(args, env, cfg_train, logdir):
    from ptask_ftr.algorithms.offrl.iql import IQL
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
...
def process_ppo_collect(args, env, cfg_train, logdir):
    from ptask_ftr.algorithms.offrl.ppo_collect import PPO, ActorCritic
    learn_cfg = cfg_train["learn"]
    is_testing = learn_cfg["test"]
    # is_testing = True
    # Override resume and testing flags if they are passed as parameters.
