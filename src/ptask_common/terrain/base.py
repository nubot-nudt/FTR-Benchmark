...
try:
    from ptask_envs.omniisaacgymenvs.utils.terrain_utils.terrain_utils import convert_heightfield_to_trimesh, SubTerrain
    from ptask_envs.omniisaacgymenvs.utils.terrain_utils import terrain_utils
...
class Terrain():
    def __init__(self, num_terains, terrain_width, terrain_length, horizontal_scale, vertical_scale,
                 slope_threshold=1.0, mesh=None):
    ...
    def set_terrain(self, index, type_name, **kwargs):
        ...
        if index != 0:
            self.heightfield[index * self.num_rows:(index + 1) * self.num_rows, :] = new_heightfield + self.heightfield[
                index * self.num_rows - 1].mean()
        else:
