...
from ptask_rlgames.loader.common import ModelLoader
from ptask_rlgames.loader.factory import LoaderFactory

from ptask_common.autonomic.executor import BaseExecutor
from ptask_common.processing.robot_info.base import RobotState
from ptask_common.processing.action_mode.define import ActionModeFactory


class RlgamesPredictor(BaseExecutor):

    @classmethod
    def load_model(cls, runs_dir, epoch=-1, device='cpu', input_shape=None, actions_num=None, action_space=None,
    ...
    def __init__(self, model_loader: ModelLoader, is_deterministic=False):
        ...
        self.action_mode = ActionModeFactory.get_action_mode_by_name(
            self.model_loader.cfg['task']['task']['actionMode'])
    ...
    def predict(self, state: RobotState):
        obs = torch.cat([state.height_map.flatten(), state.vels, torch.deg2rad(state.flippers), state.orient,
    ...
    def get_action(self, obs):
        ...
        action = self.model_loader.get_action(obs)
