...
class MapHelper():
    ...
    def get_obs(self, positon, angle, size):
        ...
        side_length = np.sqrt(size[0] ** 2 + size[1] ** 2)
        side_length = side_length // self.cell_size

        low = (pos[:2] - side_length // 2)
        up = (pos[:2] + side_length // 2)

        local_map = self.map[int(low[0]):int(up[0]), int(low[1]):int(up[1])]
        ...
        low_clip = np.array(center) - size / 2 // self.cell_size
        up_clip = np.array(center) + size / 2 // self.cell_size

        return rotated[
            int(low_clip[0]):int(up_clip[0]),
            int(low_clip[1]):int(up_clip[1])
